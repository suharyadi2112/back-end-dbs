...
// const greatAuthors = ["adi","erin"]


const filterbooks = books.filter((item) => { //filter terlebih dahulu, sales diatas 1000000
    return item.sales > 1000000;
});

// pakai array map untuk membuat array baru, dari filter sebelumnya
const greatAuthors = filterbooks.map((name) => { return `${name.author} adalah penulis buku ${name.title} yang sangat hebat!`  });
